...
let get_clients_from_env: unit => result(list(Oidc.Client.t), [ `Msg(string)]) = () => {
    open Oidc.Client;
...
let get_from_env: unit => result(t, [ `Msg(string)]) = () => {
    ...
    | (Ok(clients), Ok(provider)) => Ok({
        clients: clients,
        provider: provider
    })
    | (Error(e), Ok(_)) => Error(e)
    | (Ok(_), Error(e)) => Error(e)
    | (Error(`Msg(clients_msg)), Error(`Msg(provider_msg))) => Error(`Msg(clients_msg ++ " " ++ provider_msg))
    };
};