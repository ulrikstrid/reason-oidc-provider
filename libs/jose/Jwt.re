...
let add_claim = (claim_name: string, claim_value: Yojson.Basic.t, payload: payload) =>
  `Assoc([(claim_name, claim_value), ...Yojson.Basic.Util.to_assoc(payload)]);
...
let verify = (~jwks: list(Jwk.t), t) => {
  ...
  |> CCResult.flat_map((jwk: Jwk.t) =>
       CCResult.both(base65_url_decode(jwk.n), base65_url_decode(jwk.e))
     )
  |> CCResult.flat_map(((n, e)) => verify_internal(~n, ~e, t))
  |> CCResult.flat_map(_ => {
    let module Json = Yojson.Basic.Util;
...
let to_string = t => t.header_str ++ "." ++ t.payload_str ++ "." ++ t.signature;